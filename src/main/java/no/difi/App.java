...
public class App 
{
    ...
    public App(KeyStore.PrivateKeyEntry rootEntry, KeyStore.PrivateKeyEntry intermediateEntry) throws Exception {
        ...
        KeyStore.PrivateKeyEntry revoked = generator.generateVirksomhet("987464291", intermediate, rootCertificate, new BigInteger("7eeea5df", 16));
        ...
        KeyStore.PrivateKeyEntry missing_distribution_point = missingDistributionPoint(generator, intermediate, null);
        pkcs12.setEntry("missing_distribution_point", missing_distribution_point, protection);

        KeyStore.PrivateKeyEntry unavailable_distribution_point = unavailableDistributionPoint(generator, intermediate, null);
    ...
    private void createCrlList(KeyStore.PrivateKeyEntry intermediate, X509Certificate certificate) throws OperatorCreationException, IOException {
        ...
        builder.addCRLEntry(((X509Certificate) certificate).getSerialNumber(), DateTime.now().minusDays(10).toDate(), 0);
    ...
    private KeyStore.PrivateKeyEntry unavailableDistributionPoint(TestVirksomhetGenerator generator, final KeyStore.PrivateKeyEntry intermediate, Certificate rootCertificate) throws Exception {
        CustomCertBuilder custom = new CustomCertBuilder() {
            ...
            public void build(X509v3CertificateBuilder certGen, KeyPair keyPair) throws Exception{
                ...
                DistributionPoint distributionPoint = new DistributionPoint(
                        ...
                        new GeneralNames(new GeneralName(new X509Name(((X509Certificate)intermediate.getCertificate()).getSubjectX500Principal().getName()))));
    ...
    private KeyStore.PrivateKeyEntry missingDistributionPoint(TestVirksomhetGenerator generator, final KeyStore.PrivateKeyEntry intermediate, Certificate rootCertificate) throws Exception {
        CustomCertBuilder custom = new CustomCertBuilder() {
            @Override
            public void build(X509v3CertificateBuilder certGen, KeyPair keyPair) throws Exception{
                certGen.addExtension(Extension.authorityKeyIdentifier, false, (new JcaX509ExtensionUtils()).createAuthorityKeyIdentifier((X509Certificate) intermediate.getCertificate()));
